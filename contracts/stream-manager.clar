...
(define-public (top-up-stream
  ...
  (let (
    ...
    ;; earnings are frozen at paused-at-block, so a sender could call this repeatedly with
    ;; minimum-valid amounts just before end-block passes, pushing end-block forward
    ;; indefinitely and permanently blocking expire-stream (which requires
    ;; stacks-block-height >= end-block). Resume the stream first, then top up.
