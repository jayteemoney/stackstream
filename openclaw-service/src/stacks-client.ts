...
// Returns serialized ClarityValue args as hex strings for wallet signing
// ============================================================================

function serializeArgs(args: ClarityValue[]): string[] {
  return args.map((a) => serializeCV(a));
}
...
export function buildPauseStreamTx(streamId: number) {
  ...
  return {
    ...
    functionArgs: serializeArgs([uintCV(streamId)]),
    postConditionMode: PostConditionMode.Deny,
    network: getNetwork(),
  };
}
...
export function buildRegisterDaoTx(name: string) {
  ...
  return {
    ...
    functionArgs: serializeArgs([stringUtf8CV(name)]),
    postConditionMode: PostConditionMode.Deny,
    network: getNetwork(),
  };
}
